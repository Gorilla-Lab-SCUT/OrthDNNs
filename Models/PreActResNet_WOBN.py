...
class _DBN(nn.Module):
    ...
    def __init__(self, num_features, track_running_stats=True):
        ...
        if self.track_running_stats:
            ...
            self.register_buffer('num_batches_tracked', torch.tensor(0, dtype=torch.long).cuda())
    ...
    def forward(self, input):
        ...
        weight_expand = self.weight.expand(self.num_features)
        ...
        if self.training:
            ...
            out = weight_expand.unsqueeze(0).unsqueeze(2).unsqueeze(3) * out_ + self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        else:
            scale = weight_expand.unsqueeze(0).unsqueeze(2).unsqueeze(3) / torch.sqrt(self.running_var.unsqueeze(0).unsqueeze(2).unsqueeze(3) + self.eps)
...
def PreActResNet20_WOBN():
    return PreActResNet_CIFAR(PreActBlock, [3,3,3], 10)
